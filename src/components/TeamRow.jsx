...
export default function TeamRow({ title, members }) {
  ...
  const canSlide = members.length > VISIBLE_COUNT;

  function move(dir) {
    if (!canSlide) return;
    const max = members.length - VISIBLE_COUNT;
    setIndex((i) => {
      const next = i + dir;
  ...
  return (
    <div className="mb-14 items-center flex flex-col gap-2 sm:*: text-center">
        ...
        <div className="flex-1 overflow-hidden">
          ...
          >
            {members.map((m, i) => (
              ...
              >
                <div className="h-20 w-20 rounded-full border-2 border-bs-pink/60 bg-gradient-to-br from-[#7a1f2b] to-[#3d0f16] shadow-[0_0_12px_rgba(209,58,170,0.5)] transition duration-300 hover:scale-110 hover:border-bs-pink hover:shadow-[0_0_20px_rgba(209,58,170,0.85)] sm:h-24 sm:w-24" />
                <div>
